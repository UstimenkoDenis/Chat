...
// приставка on в названиях функций говорит что это обработчики событий
    ...
    onToggleLiked(id) { // id -  чтобы знать какой пост был отмечен
        // console.log(`Like ${id}`);
        this.setState(({data})=>{ 
        // получим номер поста наших данных
            const index = data.findIndex(elem => elem.id === id); // если наш id совпадает с тем который был передан нашей функции
            // то нашли индекс получили теперь сохраним этот пост в переменную old
            
            const old = data[index];

             //  напрямую менять нельзя поэтому создадим новую переменную в которую запишем новый объект
            ...
            // склеили то что было до индекса и
            // после а в середине наш элемент и мы получили объект в data у которого инверсировался like

            return {
                data: newArr
            }
        });          // это  один из самых сложных моментов в react 
        
        
                     //    здесь нам изначально нужно знать какой был предыдущий state true 
                    //     или false? и только в таком случае мы должны поменять наш state
                    //     поэтому мы во внутрь передаем функцию 
    }
